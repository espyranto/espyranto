...
class umolH:
    ...
    def __init__(self,plate):
        ...
        with open(f2) as f:
            umolH_ef = json.loads(f.read())
        
        self.umolH =[umolH_ef[i]['umol H2'] for i in range(len(umolH_ef))]
        seld.max = [umolH_ef[i]['Max H2 (umol)']for i in range(len(umolH_ef))]
        
        nrows = len(self.umolH)//self.plate.ncols
        ...
        dates = [datetime.strptime(os.path.split(f)[-1], f'{directory}A_y%ym%md%dH%HM%MS%S.jpg') for f in image_files]
        ...
        def __getitem__(self,index):
            ...
            elif is instance(index,tuple) and len(index) ==2:
                row,col = index
        ...
        def plot_umolH_max(self):
            ...
            plt.imshow([umolH_ef[i]['Max H2 (umol)']for i in range(len(umolH_ef))]
                      .reshape(nrows,ncols),origin = 'upper')
            plt.xlabel('columns')
            plt.ylabel('rows')
            plt.colorbar()
        
        def plot_umolH_max_contourf(self):
            mmolH = self.max.reshape(nrows,ncols)
